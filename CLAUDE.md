...
- **Fix (`ai-faq/index.ts`), all intercepted deterministically before Gemini, same pattern as the existing moisture-table/owner-assistance interceptions**:
  - **New BO-discretion detector** (`detectBoDiscretionRequest` + `BO_DISCRETION_CATEGORIES`): recognizes requests only the Business Owner can decide — early/advance payment, delivery-deadline extension, a one-off pricing/deduction exception outside the normal negotiation flow, exceeding the 3-active-contracts cap, and expedited account approval. On a match, the AI replies with an offer instead of a refusal or a promise, e.g. exactly: *"Payment arrangements are handled by NERC Copra Trading. Would you like me to inform the Business Owner so you can discuss receiving your payment earlier?"* — the AI never promises the outcome or decides anything itself, it only offers to connect the Supplier with the BO.
  - **New stateless yes/no follow-up handling**: since `ai-faq` has no per-conversation session state, "yes"/"no" is interpreted by checking whether the *immediately preceding* message in that conversation was one of these BO-discretion offers (via a stable marker substring, `"Would you like me to inform the Business Owner"`, common to every such offer) — no new column/table added. A clear "yes" (yes/sure/ok/go ahead/please do/etc.) triggers the notification; a clear "no" (no/nope/not now/no thanks/etc.) replies "Okay. If you need anything else, I'm here to help." and does **not** notify the BO. An ambiguous reply falls through to the rest of the pipeline (moisture table / another BO-discretion match / Gemini) instead of forcing a re-ask loop.
  - **Reuses the EXACT SAME escalation as "Can I talk to the Business Owner?"** — the existing inline notification logic from that interception was extracted, unchanged in behavior, into a shared `escalateToOwner()` helper (same spam-guard against duplicate unread notifications, same `notifications` insert with `notification_type: "Supplier Assistance Requested"`, same reply text on success/already-notified/failure). Both the explicit "talk to the owner" request and a "yes" to a BO-discretion offer now call this one function — there is still only one notification/escalation code path, not two.
  - Placed carefully in the existing interception order so it never overrides the deterministic moisture-content table (moisture questions are still checked first and answered from `pca_discount_table` exactly as before) — verified with a standalone regex test that "What is the moisture content deduction table?" does **not** match the new BO-discretion detector.
- **Nothing else changed**: no negotiation/counteroffer/pricing logic, no payment logic, no database schema, and no separate/duplicate notification system were introduced — this is scoped entirely to the FAQ interception layer in `ai-faq/index.ts`. No frontend changes were needed: these offer/confirmation replies are plain `is_ai_generated: true` chat messages, rendered by the exact same existing bubble/badge logic already used for every other AI FAQ reply.
- **Validated**: a standalone Deno script exercising the exact same regexes confirmed correct classification for all 6 discretion categories (payment, deadline, pricing exception, deduction waiver, contract cap, approval speed) plus correct non-matching on an unrelated moisture question and a password-reset question, and correct yes/no classification for common phrasings ("yes", "yes please", "sure", "ok", "go ahead" vs. "no", "no thanks", "nah", "not now", plus ambiguous replies matching neither). `deno check` passes with zero errors (this refactor also incidentally resolved the 2 pre-existing, unrelated `conv.supplier` array-typing warnings noted in the entry below, since the affected field access moved into a helper with a looser parameter type).
- ✅ **Deployed**: `ai-faq` was redeployed with this fix in this session (`supabase functions deploy ai-faq --no-verify-jwt`).

### 2026-09-24 — AI FAQ ("Coco") follow-up: the real reason Gemini calls were failing/slow (deprecated models + default "thinking" mode) — found via live diagnostic, deployed and verified

- **Context**: after the reliability fix below shipped (timeout + guaranteed fallback reply), the Supplier started reliably seeing the new fallback message ("I'm having trouble responding right now...") on every message — meaning the function was now behaving safely, but Gemini itself was still failing on **every single attempt**, so real answers never came through. This needed the actual root cause, not just a safety net.
- **Live diagnostic** (a temporary, unauthenticated Edge Function — `temp-diag-gemini`, deployed, exercised directly against the real `GEMINI_API_KEY`, then deleted immediately after use; never exposed the raw key, only a 4-char prefix/length) revealed two compounding problems:
  1. `gemini-2.5-flash-lite` and `gemini-2.5-flash` (2 of the previous 4 models tried) now return **HTTP 404** for this API key/project: `"This model ... is no longer available to new users."` — an upstream Google deprecation, not a bug in this codebase.
  2. The remaining `"*-latest"` alias models resolve to newer Gemini 3.x models that have **extended "thinking" (reasoning) enabled by default** — measured taking **16–46 seconds per call**, and in one case (`gemini-3.6-flash` with the default config) returning HTTP 200 with **completely empty answer text** because the entire `maxOutputTokens` budget was consumed by invisible reasoning tokens before any visible answer was produced. This perfectly explains both "too slow" and "never responds" from the original bug report, independent of anything already fixed in the timeout/fallback pass.
- **Fix (`ai-faq/index.ts`)**:
  - Replaced the model list with two models confirmed live (via repeated diagnostic calls) to be available and fast for this key: `gemini-3.1-flash-lite` (primary, ~2–4s) and `gemini-3.6-flash` (fallback, ~2–6s).
  - Added `generationConfig.thinkingConfig: { thinkingBudget: 0 }` to explicitly disable extended thinking — this is what took both models from 16–46s (or empty output) down to 2–6s consistently across repeated live test calls. (Models that don't support this field, like the old `"*-latest"` aliases, return HTTP 400 when it's present — exactly why those aliases were dropped from the model list entirely rather than kept as silently-failing retries.)
  - Raised the per-request timeout from 8s → 12s and the overall retry budget from 18s → 20s to give a safe margin above the ~2–6s now actually observed, without reintroducing unbounded worst-case latency.
  - No FAQ content, moisture-content table logic, owner-assistance interception, negotiation/contract logic, or DB schema was touched — this is scoped entirely to which Gemini model is called and how.
- **Validated**: the exact `generationConfig` used in production (including `thinkingConfig`) was exercised live against the real key multiple times for both models, consistently returning real, well-formed answer text in 2–6 seconds (vs. 0% success / 16–46s before). `deno check` shows zero new type errors (only the 2 pre-existing, unrelated `conv.supplier.first_name/last_name` warnings remain, present before this change too).
- ✅ **Deployed**: `ai-faq` was redeployed with this fix in this session (`supabase functions deploy ai-faq --no-verify-jwt`) — this correction to the "not deployed"/"cannot deploy" note in the entry below: this environment *does* have a linked, authenticated `supabase` CLI session with live deploy/secrets access, unlike some earlier sessions. All temporary diagnostic function files/deployments were deleted afterward, and no secrets were logged.

### 2026-09-24 — AI FAQ ("Coco") assistant reliability and speed fix (root cause: unbounded Gemini retries + silent failure)

- **Root cause found**: `ai-faq/index.ts`'s Gemini call had no per-request timeout at all — a single slow/hung model call could stall the whole invocation for as long as the platform allowed. On top of that, its 4-model × 2-attempt retry loop had no overall time budget, so a run of unlucky retries could take a very long time before finishing. Worse, when every model/attempt ultimately failed, the function silently returned `{ skipped: ... }` **without ever inserting a chat message** — this is the literal cause of "never responds at all": the Supplier's realtime subscription had nothing to receive, so the chat just sat there forever with no reply and no error surfaced anywhere.
- **Edge Function fix (`ai-faq/index.ts`)**:
  - Every Gemini `fetch` call is now wrapped in an `AbortController` with an 8s per-request timeout, plus an 18s overall retry budget across all models/attempts — bounding worst-case latency instead of leaving it unbounded.
  - If Gemini never returns usable text after retries (or the retry budget is exhausted), the function now inserts an honest, user-friendly fallback chat message ("I'm having trouble responding right now. Please try again in a moment, or contact NERC Copra Trading directly if this continues.") instead of silently skipping the insert — the Supplier always gets *some* reply.
  - The outer exception handler now also attempts the same fallback insert (using conversation/business-owner IDs hoisted before the `try` block) so an unexpected thrown error no longer leaves the Supplier's message unanswered either.
  - Replaced info-level `console.log` calls around actual failures (Gemini HTTP errors, fetch/timeout errors, insert failures, the final catch-all) with `console.error` (including `err.stack` where available) so real errors are visible in function logs instead of blending in with routine trace logging.
  - None of the FAQ content, the moisture-content table interception, the owner-assistance-request interception, the `ai_faq_global` toggle, or the response attribution/security checks (`verifyCaller`, RLS) were touched — only the Gemini-call reliability path and its logging changed.
- **Frontend (`NegotiationChatWidget.jsx`, `SupplierChatLayout.jsx`)**: the two fire-and-forget `supabase.functions.invoke("ai-faq", ...)` call sites (the only places `ai-faq` is triggered from) now go through a new shared helper, [aiFaq.js](/Users/reginebuenafe/Documents/School/CopTrax_Dev/CopTrax/frontend/src/utils/aiFaq.js) (`invokeAiFaq`), instead of calling `supabase.functions.invoke` directly. This is a minimal, surgical change — only these two lines (plus their imports) were touched in each file; no message-sending, proposal, negotiation, or contract logic was changed.
  - `invokeAiFaq` adds a client-side 20s `AbortController` timeout on the invoke call itself, so a hung network request can never keep anything pending indefinitely.
  - It tracks one in-flight request per `conversation_id` and skips firing a second overlapping `ai-faq` call for the same conversation while one is still pending — preventing duplicate AI requests/replies if the Supplier sends messages in quick succession.
  - It always logs the real error via `console.error` (invoke error, thrown exception, or timeout) instead of the previous behavior of completely ignoring the promise's result/rejection, and always clears the in-flight flag in a `finally` block on success, failure, timeout, or exception.
- **Validated**: `deno check` against the Edge Function's own import map shows zero new type errors (the two pre-existing `conv.supplier.first_name/last_name` array-typing warnings were already present before this change and are unrelated). `eslint` and a full frontend production build both pass clean on all three changed files plus the new helper.
- ⚠️ **Deployment note**: `ai-faq` must be redeployed (`supabase functions deploy ai-faq --no-verify-jwt`) for the timeout/fallback-reply fix to take effect live. (Update: this was in fact redeployed in this same session, along with the follow-up model fix above — see the entry above this one for what shipped and how it was verified.)

### 2026-09-23 — Inventory Report summary gains Total After Deduction and Weight Difference

- **`inventoryTotalsLines()` in `BOReportsPage.jsx`** now shows three vertically-stacked, clearly-labeled lines instead of one: **Total Net Weight**, **Total After Deduction**, and **Weight Difference** (Net − After Deduction). Same unit rules as before: Contractual figures in tons, Walk-in figures in kilograms, both to 2 decimal places; when Delivery Type is "All", per-source subtotals stay in their own units and every "Combined" figure (Net Weight, After Deduction, and Weight Difference) is converted to a common unit (tons) and explicitly labeled — tons and kilograms are never added directly. All three metrics are computed from the same unrounded per-row kg sums already used for Total Net Weight (`computeInventoryNetTotals`, extended to also sum each row's `weight_kg` as the After Deduction figure); rounding only happens in the final `fmtTons()`/`fmtKg()` display call, per the "round only for display" requirement.
- Since this is the same shared `totalsLines` value already feeding the on-screen table footer, the PDF, and the XLSX (single source of truth established in the prior report-standardization pass), all three surfaces show the new lines identically — verified by regenerating sample PDFs/XLSX for the Contractual, Walk-in, and All filters and confirming the exact same three (or seven, for "All") lines and values appear in both the extracted PDF text and the XLSX's trailing rows.
- No inventory calculations, stored data, deduction formulas, or other workflows were changed — this only extends the report's own totals-summary computation and its rendering.

### 2026-09-23 — All five BO Reports: fixed broken PDF exports, standardized XLSX styling, added Inventory Delivery Type filter + unit-aware Total Net Weight

- **Root cause found and fixed**: `exportPDF()` in `BOReportsPage.jsx` imported `jspdf-autotable` only for its side effect (`import("jspdf-autotable").then(() => {...})`) and never captured or invoked its export, so `typeof doc.autoTable === "function"` was always `false` and every PDF export silently fell back to manually `doc.text()`-ing each row joined with `"  |  "` — this was the literal source of the reported "broken, pipe-separated text". Fixed by properly importing and calling the v5 named export: `const { default: autoTable } = await import("jspdf-autotable"); autoTable(doc, {...})`.
- **Single column model drives all three surfaces**: added one column-config array per report (`CONTRACTS_COLUMNS`, `DELIVERIES_COLUMNS`, `INVENTORY_COLUMNS`, `PAYMENTS_COLUMNS`, `RATINGS_COLUMNS`), each built from small factory helpers (`textCol`, `dateCol`, `currencyCol`, `kgCol`, `tonsDirectCol`, `percentCol`, `moistureCol`, `scoreCol`). Every column's `get(row)` returns `{ text, raw, numFmt, align }` used identically by the on-screen `ReportTable`, the PDF body, and the XLSX cells — the previous implementation had three independently hand-written versions per report that had drifted out of sync (e.g. the old delivery PDF omitted the Type and Allocation Summary columns present in XLSX/screen; payments PDF omitted Method; contracts PDF omitted Email/Signing Date). All three surfaces now show the exact same records/columns/totals for every report.
- **PDF formatting overhaul** (`exportPDF`): real `jspdf-autotable` tables with header fill/bold, alternating row shading, thin gridlines, consistent margins, `cellPadding`, automatic page breaks (verified with a 60-row/3-page synthetic table), and real "Page X of Y" footers added only after final pagination is known (avoids the classic off-by-one from numbering during `didDrawPage`).
- **Peso-glyph PDF bug found and fixed**: jsPDF's built-in "helvetica" font has no ₱ (U+20B1) glyph — every currency cell rendered as garbled character soup (confirmed via extracted PDF text: `"± 3 7 . 0 0"` instead of `"₱37.00"`). Added a PDF-only `pesoPdf()` formatter ("PHP 37.00") used solely for PDF cell/body text (via a new `pdfText` field, falling back to `text` when absent); the on-screen table and XLSX numeric format keep the real ₱ symbol since browsers/Excel render it fine.
- **XLSX styling overhaul**: switched this file's XLSX generation from plain `xlsx` (SheetJS Community Edition, which silently drops cell styles on write) to the MIT-licensed `xlsx-js-style` fork (added as a new dependency; the original `xlsx` package is untouched and still used elsewhere, e.g. `PaymentsPage.jsx`/`SupplierPaymentsPage.jsx`). Every export now has a bold white-on-green header row with thin borders, thin-bordered body cells, sensible column widths, a merged title/subtitle block, and real typed cells (numbers/Excel date serials with `numFmt`, not pre-formatted strings) for currency, weight, percent, moisture, and date columns — verified by unzipping the actual `.xlsx` output and inspecting the raw OOXML (`styles.xml`/`sheet1.xml`) directly, since `xlsx-js-style`'s own reader doesn't fully reconstruct styles on read-back (write-side only; irrelevant to this feature, which never re-reads its own exports).
- **Inventory Report — new Delivery Type filter**: reused the exact same "All / Contractual / Walk-in" UI pattern already built for the Delivery Report (`deliveryTypeFilter` state, now shared by both reports since only one report is active at a time) — filtering on `source_type` for Inventory vs `delivery_source` for Deliveries. Applied identically to the screen table, PDF, and XLSX.
- **Inventory Report — new Total Net Weight**: `fetchInventory()` now also joins `weighing_records(net_weight_kg)` so `inventoryNetWeightKg()` can compute each batch's pre-deduction Net Weight (Gross − Tare), not the post-PCA `weight_kg` (still shown separately as its own "After Deduction" column). `inventoryTotalsLines()` computes the total from every fetched row (not just the currently-filtered subset) split by `source_type`, so switching the filter always shows the mathematically correct total for that scope: "Contractual" → total in tons only; "Walk-in" → total in kg only; "All" → separate Contractual (t) and Walk-in (kg) subtotals plus one combined total converted to a common unit (tons) and explicitly labeled — tons and kilograms are never added directly. Rejected deliveries structurally can't appear (`inventory_batches` rows are only ever inserted for `Accepted` deliveries, per `InspectionQueuePage.jsx`) and multi-contract batches can't double-count (each delivery produces exactly one `inventory_batches` row keyed by `delivery_id`, independent of how many contracts/allocations it was split across).
- **Weight units per row**: `inventoryWeightCol()` displays Contractual-sourced batches in tons and Walk-in-sourced batches in kilograms (both 2-decimal), consistently across the screen table, PDF, and XLSX (XLSX stores the raw number in the row's own unit with a matching `numFmt`, so Excel's SUM()/sort still works meaningfully per unit).
- **Verified end-to-end for all five reports, both formats**, using a temporary Node test harness that imported the actual column-config/export-function code from this file (not a re-typed copy) with realistic sample rows matching each `fetchX()`'s exact nested-join shape: confirmed zero pipe-separated fallback text, zero garbled currency glyphs, correct multi-page pagination, and — for Inventory specifically — the exact expected totals (11,500 kg contractual net + 1,000 kg walk-in net = 12,500 kg / 12.50 t combined) matching identically between the PDF and XLSX outputs. All temporary test files were removed afterward.
- No delivery records, moisture deductions, payment calculations, contract logic, or delivery workflows were touched — this is entirely a report-generation/display fix plus the two new Inventory Report features requested.

### 2026-09-23 — Owner Inventory "Resecada Pool" renamed to "Bodega Stock", capacity progress bar, 80%/100% notifications

- **Rename (UI only)**: every user-facing "Resecada Pool" label on the Owner Inventory page (`InventoryPage.jsx`) — the tab, the header subtitle, the summary card, the total-stock banner, the empty state, and the `ResecadaTab` component (renamed to `BodegaStockTab`) — now reads **"Bodega Stock"**. The underlying `inventory_batches.batch_status = 'Resecada'` DB enum value and all queries filtering on it are unchanged; only display text/component names changed.
- **Aggregation basis switched to Net Weight**: Bodega Stock's summary-card total and "Total Bodega Stock" banner now sum each batch's own delivery **Net Weight** (`weighing_records.net_weight_kg`, Gross − Tare) via a shared `getNetWeightKg()` helper, instead of the post-PCA-deduction `inventory_batches.weight_kg` ("After Deduction", still shown as its own column in the table, unchanged). Rejected deliveries never produce `inventory_batches` rows at all (confirmed in `InspectionQueuePage.jsx` — the insert only happens on `preview.result === "Accepted"`), and each delivery produces exactly one `inventory_batches` row regardless of how many contracts it was split across (keyed by `delivery_id`, not by allocation), so this sum structurally cannot include rejected deliveries or double-count a multi-contract batch.
- **Capacity + progress bar**: capacity is **500 tons (500,000 kg)** — the real, already-agreed figure from the existing `warn_inventory_capacity()` trigger (migrations `20260912000051`–`53`), not an invented number; centralized as `BODEGA_CAPACITY_KG` in `InventoryPage.jsx` with a comment to keep it in sync with the DB trigger if the physical capacity ever changes. The Bodega Stock summary card gained a progress bar (`SummaryCard`'s new `capacityPercent`/`capacityLabel`/`barColor` props) showing percent used, tons used/remaining, and a color that shifts green → amber ("Almost Full") → red ("Full"). A matching on-page warning banner appears above the summary cards at the same 80%/100% thresholds. The Walk-in Holding card is unchanged (no capacity concept).
- **DB notifications — migration `20260923000071_bodega_stock_capacity_notifications.sql`**: rewrites `warn_inventory_capacity()` to (1) sum Net Weight instead of `weight_kg`, scoped to `batch_status = 'Resecada'` only (matching the renamed page exactly, instead of the previous broader 3-status sum), and (2) add the previously-missing **100% "Full"** threshold alongside the existing 80% "Almost Full" one — both reuse the same `previous < threshold AND current >= threshold` crossing-edge technique already in place, so each threshold notifies only once per genuine crossing and cannot duplicate while stock stays at/above it (verified below). Still reuses the existing `'Inventory Capacity Warning'` notification type (no new enum value) and the existing `related_entity_type = 'inventory_capacity'`, so `NotificationBell.jsx` needed no changes.
- **Validated against a throwaway local Postgres cluster** (minimal stub schema for `users`/`roles`/`deliveries`/`weighing_records`/`inventory_batches`/`notifications`, migration's function/trigger applied as-is): confirmed the total correctly uses Net Weight (not `weight_kg`) by cross-checking the exact tonnage in the resulting notification messages; confirmed "Almost Full" fires exactly once at the 400t crossing and does not re-fire as stock continues rising through 450t; confirmed "Full" fires exactly once at the 500t crossing; confirmed that manually moving batches out of `'Resecada'` (dropping stock back below 400t) and then back in correctly re-fires "Almost Full" on the next genuine upward crossing (no permanent one-shot lockout, no duplicate spam while sitting above threshold).
- No delivery records, moisture deductions, payment calculations, contract allocations, or existing workflows were touched — this is scoped entirely to inventory aggregation, capacity display, and notification functionality, per the request.
- ⚠️ **Deployment note**: apply migration `20260923000071` (same sandbox limitation as always — this environment cannot reach the live Supabase DB directly; use `supabase db push` or the SQL Editor). No Edge Function redeploy is needed — this change is a single DB function/trigger replacement plus the paired frontend page.

### 2026-09-23 — Owner Inventory Resecada Pool table shows Net Weight alongside After Deduction

- **Owner Inventory (`InventoryPage.jsx`)**: the Resecada Pool table's `Weight (t)` column was renamed to **"After Deduction (t)"** (its values are completely unchanged — still `inventory_batches.weight_kg`, the post-PCA-deduction Final Weight). A new **"Net Weight (t)"** column was added immediately before it, showing the delivery's own pre-deduction `weighing_records.net_weight_kg` (Gross − Tare, already stored at weighing time). Both columns use the same `fmtTons()` helper (tonnes, 2-decimal precision) and identical `px-5 py-3.5` cell spacing/alignment as the rest of the table, in both the desktop table and the mobile stacked-card view. Only the Resecada query's `delivery` embed gained one additive nested field (`weighing_records(net_weight_kg)`); no inventory totals, stock computations, delivery records, deduction formulas, or backend logic were touched. Walk-in Holding tab is unaffected.

### 2026-09-23 — Owner Dashboard summary cards are clickable (deep-link into User Management / Payments)

- **Owner Dashboard (`OwnerOverview.jsx`)**: the "Pending Approvals" stat card was renamed to **"Pending Registrations"** for clarity (its underlying count is unchanged — still `users.account_status = 'Pending'`, i.e. Supplier account approvals, not contract approvals) and is now clickable, navigating to `/dashboard/owner/users`; that page's own `tab` state already defaults to `"Pending"`, so no new filter-param logic was needed there. The **"Pending Payments"** card is now clickable too, navigating to `/dashboard/owner/payments?tab=batches&filter=Pending` — matching this stat's own query (`payments.payment_status = 'Pending'`) exactly. Non-clickable cards ("Active Contracts", "Total Amount Paid") are unchanged.
- **Owner Payments (`PaymentsPage.jsx`)**: now reads `?tab=batches&filter=Pending` (via `useSearchParams`, read once on mount) to open directly on the existing "Payment Batches" tab with its existing "Pending" sub-filter pre-selected (`BatchesTab`'s local `filter` state now accepts an `initialFilter` prop). No new tabs, filters, or payment logic were added — this reuses the exact existing tab/filter UI, just deep-linked into.
- Resolved via clarification with the user: the dashboard's "Pending Approvals" card was confirmed to represent Supplier registration approvals (not contract approvals), so it deep-links to User Management rather than the Contracts page. No contract-related dashboard card/link was added as part of this change.

### 2026-09-23 — Bank Details added to Contractual payment receipts (masked account number)

- **Owner Payments (`BatchReceiptModal` in `PaymentsPage.jsx`)**: the Contractual e-receipt now includes a "BANK DETAILS" section (Bank Name, Account Name, and a masked Account Number, e.g. `••••••1234`) sourced from the supplier's existing `bank_accounts` row. The `payments` fetch query now also embeds `supplier:supplier_id(..., bank_accounts(bank_name, account_name, account_number))`; BO read access to any supplier's `bank_accounts` row was already granted by the existing `bank_accounts_bo_select` RLS policy (migration `20260918000069`), so no new database/RLS changes were needed. A new `maskAccountNumber()` helper always keeps only the last 4–5 digits visible and pads the rest with at least six `•` characters — the full account number is never rendered. This section is only added to the Contractual `BatchReceiptModal`; the separate `WalkinReceiptModal` (Walk-in receipts) is untouched and shows no bank details, since Walk-in payments are never bank transfers. All existing payment calculations, payout logic, receipt amounts, and workflows are unchanged — this is a pure display addition plus one additive query field.

### 2026-09-23 — Supplier and Owner Deliveries filter tabs simplified to All/Pending/Accepted/Rejected

- **Supplier Deliveries (`SupplierDeliveriesPage.jsx`)**: removed the separate "Weighed" and "Inspected" filter tabs — `FILTERS` is now just `["All", "Pending", "Accepted", "Rejected"]`. `supplierStatusKey()` was simplified so any delivery not yet `Accepted`/`Rejected` (whether truly untouched, weighed-but-unassessed, or the rarely-used raw `"Inspected"` DB status) always resolves to `"Pending"` — matching the actual current lab workflow, where `InspectionQueuePage.jsx` only ever pulls deliveries still at `delivery_status = "Weighed"` and writes the moisture assessment and the final `Accepted`/`Rejected` decision in the same action (there is no real intermediate "assessed but undecided" state to preserve). The status badge shown per delivery now reads "Pending" for any weighed-but-unassessed delivery instead of the old separate "Weighed"/"Assessed" badges. The filter dropdown/tabs both reuse this same `supplierStatusKey()` function (previously the filter compared the raw DB `delivery_status` directly), so filtering and the displayed badge can never disagree. No delivery statuses, moisture-assessment logic, database records, or Accept/Reject workflows were changed — this is a pure frontend grouping/label change.
- **Owner Deliveries (`BODeliveriesPage.jsx`)**: applied the identical simplification. `FILTERS` is now `["All", "Pending", "Accepted", "Rejected"]` and a new `boDeliveryStatusKey()` helper collapses any raw `delivery_status` other than `Accepted`/`Rejected` (i.e. `Pending`, `Weighed`, or `Inspected`) into `"Pending"` for both the tab filter and the status badge lookup (previously the filter/badge read `d.delivery_status` directly, so a `Weighed` delivery had its own separate tab/badge). Same scope as the Supplier page — pure frontend filter/label change, no delivery records or lab workflow touched.
- **Owner Deliveries — status filters hidden for Walk-in**: the mobile status `<select>` and desktop underline tabs are now only rendered when the Contractual toggle is active (`activeDeliveryType === "Contract-based"`); switching to Walk-in hides them entirely and the `filtered` list bypasses the status check altogether for Walk-in, showing every Walk-in delivery regardless of the (hidden) `filter` state. Matches the existing business rule that Walk-in deliveries never go through the Weighed/Inspected/Accepted/Rejected lab pipeline. The Contractual tab's filters, sorting, search, and all delivery data/calculations are unchanged.

### 2026-09-22 — Three display fixes: rejected-delivery allocation panel, supplier card stats, batches modal total

- **Supplier Deliveries (`SupplierDeliveriesPage.jsx`)**: the Allocation Breakdown section is now completely hidden for deliveries whose computed `displayStatus` is `"Rejected"` (reuses the same status value already driving that row's status badge). Non-rejected deliveries with allocations are unaffected.
- **Owner Deliveries (`BODeliveriesPage.jsx`)**: the same Allocation Breakdown section (present on this page too, not just the Supplier's) is now also hidden when `delivery_status === "Rejected"`. The Owner Payments Preview's own Allocation Breakdown (`PaymentsPage.jsx`) already only ever queries `Accepted` deliveries, so it was never affected.
- **Owner Contracts supplier cards (`BOContractsPage.jsx`)**: removed the "Awaiting Review" (`Pending Owner Review`) stat from each supplier's summary card on the landing list — reverting `SUPPLIER_STAT_ORDER`/`SUPPLIER_STAT_LABELS`/`SUPPLIER_STAT_ICON_BG`/`SUPPLIER_STAT_DOT` to just Active/Completed/Breached (their values are unchanged, just no longer shown here). The Contracts/Active/Completed/Breached/View cluster is pinned flush to the card's right edge (`sm:ml-auto`) as one fixed-width unit with its original tight `gap-x-5` spacing, so it lines up consistently across every supplier card regardless of name length — without the earlier `1fr`-based CSS grid attempt, which stretched the gaps out with excess whitespace on real (wider) card widths. Verified visually via an isolated static Tailwind render at both narrow and app-realistic widths (temporary test files removed afterward). The system-wide pending banners and the "Pending Contracts" side panel added earlier this session are unaffected.
- **Owner Contracts Delivery Batches modal (`DeliveryBatchesModal`/`OwnerBatchTable` in `BOContractsPage.jsx`)**: the former "Net Weight" column was renamed to **"Allocated Weight"** and now shows each batch's `delivery_allocations.allocated_weight_kg` — i.e. only the portion of that delivery actually allocated to **this** contract — instead of the delivery's raw Net Weight, matching the same per-contract figure already shown in that delivery's own Allocation Breakdown on the Deliveries pages (e.g. Batch 3's 8,500 kg net delivery is cascade-split 7,000 kg to this contract + 1,100.50 kg to Spot, so this column and the footer both now show 7,000.00 kg for that batch instead of the previously-mismatched 8,500.00 kg). Gross/Tare Weight remain the delivery's own physical measurements (not contract-specific) and are unchanged. The footer's "N tons total" sums this same allocated-weight column for Accepted batches only (10.00 tons for this example), staying internally consistent with the table above it. Rejected batches remain visible in the table and still count toward the "N batches" label but are excluded from the tons total. No contract statuses, delivery records, allocation logic, or payment calculations were touched — this is a display-only column/sum correction.
- **Moisture unit corrected to "cc"**: both the Owner's Delivery Batches modal (`OwnerBatchTable` in `BOContractsPage.jsx`) and the Supplier's own Delivery Batches modal (`batchMoisture()` in `MyContractsPage.jsx`) were displaying moisture content with a "%" suffix; both now display "cc" (e.g. `10.0cc`), consistent with the moisture-content unit already used everywhere else in the app (PCA discount table, Lab Inspection Queue, AI FAQ's moisture table). Purely a unit-label display change — the underlying `moisture_content_pct` value and all PCA-deduction/business logic are unchanged.

### 2026-09-22 — Supplier Pending contracts tab includes owner-review contracts

- **Supplier My Contracts (`MyContractsPage.jsx`)**: the Pending tab now includes contracts with both `Pending` and `Pending Owner Review` statuses and displays counts for tabs that contain contracts. Contract statuses, workflows, and all other filters remain unchanged.
- **Responsive toolbar**: status tabs now wrap instead of clipping long labels such as Breached, and search/sort controls remain below the tabs until the extra-wide layout breakpoint.
- **Owner Contracts (`BOContractsPage.jsx`)**: merged the separate Pending and Pending Review tabs into one Pending tab with UI-only All Pending, Supplier Signature, and Owner Review sub-filters mapped to the existing statuses. Contract workflows, signature requirements, and backend logic are unchanged.
- **Owner Contracts layout cleanup**: the main status tabs (All/Pending/Active/Completed/Breached) now stay on a single non-wrapping row (horizontal scroll only kicks in on narrow mobile widths), and the Pending sub-filter pills render on their own left-aligned row directly below the tabs instead of being squeezed onto the same row as search/sort, removing the uneven spacing and stray wrap seen with "Breached" previously. Counts, filtering logic, and statuses are unchanged.
- **Owner Contracts system-wide Pending visibility**: the Pending tab/sub-filters are still scoped to the currently opened supplier (unchanged architecture — the page is supplier-drill-down by design), but the BO can now see pending items across *all* suppliers without opening each one individually. Each supplier card on the landing list now also shows an "Awaiting Review" count (status `Pending Owner Review`, only rendered when non-zero, alongside the existing always-shown Active/Completed/Breached counts). Two dashboard banners ("N contract(s) awaiting the Supplier's signature" / "N contract(s) signed by the Supplier and awaiting your review & approval") are always computed system-wide (across every supplier, not just the one currently open). No contract statuses, workflows, or backend logic changed — this only surfaces already-fetched data more visibly.
- **Owner Contracts pending-contracts panel**: clicking either dashboard banner now opens a right-side slide-in panel (`PendingContractsPanel`) listing every system-wide pending contract, grouped into "Awaiting Supplier Signature" and "Awaiting Owner Review". Each row shows supplier name, contract number, and status badge, with a "View" button. "View" reuses the page's existing `openContract()` router unchanged — for a Pending Owner Review contract this still opens the same `ContractApprovalModal` approval workflow; for an ungenerated Pending contract it still opens the same `ContractReviewModal`; otherwise it opens the same PDF viewer. The panel is scrollable, responsive (full-width on mobile, capped width on larger screens), and has its own close button plus a backdrop-click-to-close. The panel and its backdrop now animate in/out (slide-from-right + fade) via `AnimatePresence` and two new shared `MotionAside`/`MotionButton` aliases added to `motion-elements.js` (same pattern as the existing `MotionDiv`). No new contract statuses, approval logic, or backend workflows were introduced.
