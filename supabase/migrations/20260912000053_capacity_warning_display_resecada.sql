-- Keep the threshold based on all active inventory, but display the Resecada
-- pool volume requested by the Business Owner in the warning message.
  ...
  IF v_previous_kg < v_warning_kg AND v_current_kg >= v_warning_kg THEN
    ...
    IF v_owner_id IS NOT NULL THEN
      ...
      VALUES (
        ...
        'Inventory capacity: ' || ROUND(v_display_kg / 1000)::INTEGER || 't / ' || ROUND(v_capacity_kg / 1000)::INTEGER || 't. Consider selling soon.',
        'inventory_capacity'
      );
    END IF;
  END IF;
...
BEGIN
  ...
  SET message = 'Inventory capacity: ' || ROUND(v_resecada_kg / 1000)::INTEGER || 't / 500t. Consider selling soon.'
